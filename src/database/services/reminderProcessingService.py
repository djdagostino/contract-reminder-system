...
class ReminderProcessingService:
    ...
    def get_unprocessed_emails(self):
        try:
            unprocessed_emails = self.session.query(ReminderProcessing).filter(ReminderProcessing.IsExpReminderSent == 0).all()
    ...
    def mark_email_as_sent(self, reminder_id: int):
        try:
            self.session.query(ReminderProcessing).filter(ReminderProcessing.ReminderId == reminder_id).update({"IsExpReminderSent": 1})
            self.session.commit()
        except SQLAlchemyError as e:
            self.session.rollback()
